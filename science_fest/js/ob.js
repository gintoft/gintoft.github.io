...
function drawCard(name, date, info, place, type, client, classList, TimePadID) {
    ...
    html += '<br><br><a href="https://festivalnaukivnso.timepad.ru/event/' + TimePadID + '/#register" target="_blank" class="btn btn-success">Зарегистрироваться!</a>'
    html += '</div></div><br>';
...
function fillCardholder(){
    ...
    for (i = 0; i < dataset.length; i++){
        drawCard(dataset[i].name, dataset[i].date, dataset[i].info, dataset[i].place, dataset[i].type, dataset[i].client, dataset[i].classList, dataset[i].TimePadID);
    }
